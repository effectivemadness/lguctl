...
BUILD_DIR ?= ./out
COMMAND_PKG ?= cmd
ORG = github.com/u-cto-devops
PROJECT = lguctl
VERSION = 0.0.2
REPOPATH ?= $(ORG)/$(PROJECT)
RELEASE_BUCKET ?= uplus-cto-devops-files
...
amp_TEST_PACKAGES = ./pkg/... ./cmd/... ./hack/...
...
$(BUILD_DIR)/$(PROJECT)-%: $(STATIK_FILES) $(GO_FILES) $(BUILD_DIR) deploy/cross/Dockerfile
	...
	docker run --platform linux/amd64 --rm lguctl/cross cat /build/lguctl > $@
