...
import (
	...
	"github.com/u-cto-devops/lguctl/pkg/builder"
	"github.com/u-cto-devops/lguctl/pkg/color"
	"github.com/u-cto-devops/lguctl/pkg/config"
	"github.com/u-cto-devops/lguctl/pkg/constants"
	"github.com/u-cto-devops/lguctl/pkg/execute"
	"github.com/u-cto-devops/lguctl/pkg/keymanager"
	"github.com/u-cto-devops/lguctl/pkg/schema"
	"github.com/u-cto-devops/lguctl/pkg/templates"
...
func (r Runner) Setup(out io.Writer, args []string) error {
	...
	if len(r.Config.Alias) > 0 {
		if _, ok := r.Config.Alias[target]; ok {
			arn = r.Config.AssumeRoles[r.Config.Alias[target]]
			target = r.Config.Alias[target]
		}
	}
	...
	applyToCliProfile := viper.GetBool("apply-to-cli-profile")

	if applyToCliProfile {
		exec.Command("aws", "configure", "set", "aws_access_key_id", *assumeCreds.AccessKeyId, "--profile", target).Run()
		exec.Command("aws", "configure", "set", "aws_secret_access_key", *assumeCreds.SecretAccessKey, "--profile", target).Run()
		exec.Command("aws", "configure", "set", "aws_session_token", *assumeCreds.SessionToken, "--profile", target).Run()
	}
	...
	if rawOutput {
		...
		err = tools.CopyToClipBoard(
			fmt.Sprintf(`export AWS_ACCESS_KEY_ID=%s
export AWS_SECRET_ACCESS_KEY=%s
export AWS_SESSION_TOKEN=%s
`, *assumeCreds.AccessKeyId, *assumeCreds.SecretAccessKey, *assumeCreds.SessionToken),
		)
		if err != nil {
			return err
		}
...
func (r Runner) StartMaintenance(out io.Writer, args []string) error {
	...
	maintenanceMessage := r.Config.Maintenance.Message
	targetLBList := r.Config.Maintenance.Arns

	for _, targetLB := range targetLBList {
		listeners, err := r.AWSClient.DescribeListeners(targetLB.LoadbalancerArn)
		if err != nil {
			return err
		}

		color.Red.Fprintln(out, "Start maintenance for "+targetLB.LoadbalancerArn)
		if err := tools.AskContinue("Are you sure to start maintenance? "); err != nil {
			return errors.New("job has been cancelled")
		}

		//_, err = r.AWSClient.ModifyListenerForStartingMaintenance(listeners[0].ListenerArn, maintenanceMessage)
		if err := r.AWSClient.CreateMaintenanceRule(listeners[0].ListenerArn, maintenanceMessage); err != nil {
			color.Red.Fprintln(out, "FAILED")
			return err
		}
		color.Green.Fprintln(out, "SUCCESS")
	}
...
func (r Runner) StopMaintenance(out io.Writer, args []string) error {
	...
	for _, targetLB := range targetLBList {
		...
		color.Blue.Fprintln(out, "Stop maintenance for "+targetLB.LoadbalancerArn)
		if err := tools.AskContinue("Are you sure to stop maintenance? "); err != nil {
			return errors.New("job has been cancelled")
		}

		listenerArn := listeners[0].ListenerArn
		rules, err := r.AWSClient.DescribeRules(listenerArn)
		if err != nil {
			return err
		}
		...
		for _, rule := range rules {
			if rule.Priority != nil {
				...
				if err == nil {
					if priority == constants.DefaultMaintenancePriority {
						for _, con := range rule.Conditions {
							if con.PathPatternConfig != nil && *con.PathPatternConfig.Values[0] == "/*" {
								if err := r.AWSClient.DeleteMaintenanceRule(rule.RuleArn); err != nil {
									return err
								}
								color.Blue.Fprintf(out, "Deleted: %s, %s", *rule.RuleArn, *con.PathPatternConfig.Values[0])
							}
						}
					}
				}
...
func (r Runner) RegisterCredentialToKeyChain(_ io.Writer) error {
	k, err := keymanager.GenerateKeyManager(constants.DefaultKeyType)
	if err != nil {
		return err
	}
...
func (r Runner) Exec(out io.Writer, a *args.Argument) error {
	...
	if r.Flag.Duration > 0 {
		r.Config.Duration = r.Flag.Duration
	}

	creds, err := k.Generate(a.Profile, r.Config)
	if err != nil {
		return err
	}

	return execute.Execute(creds, out, a)
}
...
func (r Runner) GetIAMGroupForUser(out io.Writer) error {
	cfg, err := config.GetConfigWithoutSelect()

	if err != nil {
		return err
	}

	if err := r.AWSClient.GetListGroupForUser(out, cfg[0].Name); err != nil {
		return err
	}
...
func (r Runner) GetIAMPolicyAttachedGroup(out io.Writer) error {
	...
	if err := r.AWSClient.GetListPolicyAttachedGroup(out, cfg[0].Name); err != nil {
		return err
	}
...
func (r Runner) GetIAMRoleArnForUser(out io.Writer) error {
	...
	if err := r.AWSClient.GetListRoleArn(out, cfg[0].Name); err != nil {
		return err
	}
